...
public class GBreadboard extends JPanel {
	...
	setPreferredSize(new Dimension((getColumnCount() - 1) * 200 + 30,
		(getRowCount()) * 50 + 30));
	setLayout(null);
	...
	try {
	    ...
	    System.out
		    .println("Trying to add more Gates than the breadboard can fit!!!");
	}
    }
	...
	if (mBreadboard.sockets != null) {
		...
		for (int y = 0; y < getRowCount() - 1; y++) {
			...
			for (int row : sock.inputPinNumbers) {
			    int x1 = getPinLocation(sock.column, row).x;
			    int y1 = getPinLocation(sock.column, row).y;
