    ...
    public GatePicker(GBreadboard parent, int col) {
        ...
        bOk.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                if (column >= 0)
                    parentBreadboard.addSocket(
                            (Type) comboGate.getSelectedItem(), column);
                else parentBreadboard.addSocket(
                        (Type) comboGate.getSelectedItem(),
                        (Integer) comboColumn.getSelectedItem());
