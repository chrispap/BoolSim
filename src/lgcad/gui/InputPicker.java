...
public class InputPicker extends JFrame{
	...
	public InputPicker(GSocket gate) {
		...
		for(int i=0; i<rows; i++) pinNumbers[i] = i+1;
		
		JButton bOk = new JButton("OK");
		bOk.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent evt) {
				gSocket.makeConnections((Integer)input1.getSelectedItem()-1, (Integer)input2.getSelectedItem()-1, (Integer)output.getSelectedItem()-1);
				setVisible(false);
			}
		});
